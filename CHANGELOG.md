...
* **Reflectance reversal** ([#96](https://github.com/koji-1009/yomu/issues/96)): QR codes printed as light modules on a dark background (ISO/IEC 18004:2015, 6.2) now decode at every `DecodeEffort`, in both `decode` and `decodeAll`. The finder pattern scan reads light-on-dark patterns from the same runs as the normal ones - a 1:1:3:1:1 sequence that starts on a white run - so the image is scanned once; those candidates are tried once the normal attempt (and, in `decode`, barcode scanning) has failed. Decoding a normal code costs up to 3% more (`qr_images` at `fast`: 0.43ms to 0.44ms). Frames holding no code pay for checking the light-on-dark candidates: on a textured Full HD frame, `fast` goes from 2.5ms to 3.6ms (measured with the #94 fix), `balanced` from 14.8ms to 15.9ms and `thorough` from 61.5ms to 63.8ms. Blank frames are unaffected.
  * `readLightOnDark` (default `true`) turns it off, and with it every one of these costs: a textured Full HD frame holding no code is about 0.9ms faster at `fast` and 1ms at `balanced`.
  * The retry stages read light-on-dark codes too: despeckle at `balanced`, and the full-resolution retry and threshold sweep at `thorough`, each from the finder patterns its own row scan collects. Light-on-dark candidates get the corner-grid rescue in the first stage only, as the rescue shares one work budget. With every QR fixture colour-inverted, `balanced` reads 163 of 182 (was 150) and `thorough` 166 (was 150), against 169 and 173 for the originals. `thorough` pays for it on frames holding no code: a textured Full HD frame goes from 65ms to 75ms.
  * `decodeAll` returns every code on a sheet mixing dark-on-light and light-on-dark ones; light-on-dark codes were read only when no dark-on-light one decoded. Its threshold sweep and full-resolution passes leave light-on-dark candidates out: picking disjoint triplets costs the cube of the candidate count, and a textured frame at full resolution holds hundreds of them.
