...
void main() {
  group('BitSource', () {
    test('reads single bits correctly', () {
      // 0b10110100 = 180
      final source = BitSource(Uint8List.fromList([180]));
    ...
    test('reads multiple bits correctly', () {
      // 0b11001010 = 202
      final source = BitSource(Uint8List.fromList([202]));
    ...
    test('reads across byte boundaries', () {
      ...
      final source = BitSource(Uint8List.fromList([0xFF, 0x00]));
    ...
    test('available() returns correct count', () {
      final source = BitSource(Uint8List.fromList([0xFF, 0xFF, 0xFF]));
    ...
    test('reads 32 bits correctly', () {
      // 0x12345678
      final source = BitSource(Uint8List.fromList([0x12, 0x34, 0x56, 0x78]));
      expect(source.readBits(32), 0x12345678);
    });

    test('byteOffset and bitOffset are correct', () {
      final source = BitSource(Uint8List.fromList([0xFF, 0xFF]));
